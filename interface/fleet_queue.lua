...
--
-- ONE ROW PER SUBSCRIPTION, each with the provider's name, a bar and its
-- percentage. The account may hold several and they are separate windows on
-- separate clocks, so nothing here is summed across them and the name is what
-- keeps three readings from being read as one. The bar is a SECOND encoding of
-- the number beside it, coloured against the reserve that arrives on the
-- record and marked where that floor falls — never a replacement for the
-- number, and never drawn for a reading that is missing rather than spent.
--
...
---
--- ONE ROW PER PROVIDER, each with its name, a bar and its percentage. The
--- name is not decoration: three subscriptions drawn without one are three
--- numbers that read as one reading with two mistakes in it.
---
...
--- cells it is a decoration and the number is the reading — then the reason on
--- an unavailable row, which truncates. The number never goes.
---
...
---
--- THREE READINGS ARE NOT BARS. No record yet is the spinner; a provider that
--- could not be read says `unavailable` with its reason, because an empty bar
--- there would read as a spent window rather than a missing one; and a stale
...
local function fuel_rows(fuel, width, spinner)
  ...
  if not fuel or #fuel == 0 then
    local row = ui.row({ width = width })
    row:add(lead, { fg = theme.muted })
    row:add(spinner .. " reading", { fg = theme.muted })
    return { line(row:spans_list()) }
  end
  ...
  if age then
    local pad = width - head.used - widgets.len(age)
    if pad >= 2 then
      head:add(string.rep(" ", pad))
      head:add(age, { fg = theme.muted })
    end
  end

  local rows = { line(head:spans_list()) }

  local label_width, readings, only = 1, 0, nil
  for _, rec in ipairs(fuel) do
    ...
    if rec.remaining and not rec.unavailable then
      readings, only = readings + 1, rec
    end
  end
  label_width = math.min(label_width, FUEL_LABEL_MAX)
  ...
  for _, rec in ipairs(fuel) do
    ...
    row:add(widgets.pad(widgets.truncate(rec.provider or "", label_width), label_width),
      { fg = theme.muted })
    row:add(" ")
    if rec.unavailable or not rec.remaining then
      -- An unreadable reading is drawn as unreadable, with the reason the
      -- probe gave — never as an empty bar, which would read as a spent
      -- window rather than a missing one.
      row:add(widgets.truncate_hard("unavailable", math.max(0, width - row.used)),
        { fg = theme.warn })
      local left = width - row.used - 1
      if left >= 4 then
        row:add(" " .. widgets.truncate_hard(rec.unavailable or "no reading", left),
          { fg = theme.muted })
      end
    else
      ...
      local cells = width - row.used - widgets.len(number) - 1
  ...
  if readings == 1 then
    ...
    for _, level in ipairs(FUEL_DETAIL) do
      local segs = detail_segments(only, level)
      if #segs == 0 then
        break
      end
